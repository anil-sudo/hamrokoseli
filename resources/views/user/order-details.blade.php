        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-semibold text-(--text-color) inline-flex items-center">
                    Order Details
                    <span class="ml-3 px-4 py-1 text-sm font-medium {{ $badgeColor }} text-white rounded-full capitalize">
                        {{ $order->status }}
                    </span>
                </h1>
                <div class="flex items-center gap-3 text-sm mt-1">
                    ...
                    <span class="text-(--text-dark)/50">Placed on {{ $order->created_at->format('M d, Y') }} at {{ $order->created_at->format('h:i A') }}</span>
                </div>
            </div>
        </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-(--card-bg) rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
                    ...
                    @else
                            ...
                            @foreach ($steps as $i => $step)
                                ...
                                <div class="flex lg:flex-col items-center text-center">
                                    ...
                                    <div class="ml-4 lg:ml-0 lg:mt-2">
                                        ...
                                        @if ($done && $i === 0)
                                            <p class="text-xs text-(--text-dark)/55">{{ $order->created_at->format('M d, Y') }}<br>{{ $order->created_at->format('h:i A') }}</p>
                                        @endif
                                    </div>
                                </div>
                        ...
                        @if ($msg)
                            <div class="mt-6 bg-{{ $msg['color'] }}-50 border border-{{ $msg['color'] }}-200 rounded-xl p-4 flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-{{ $msg['color'] }}-100 flex items-center justify-center">
                                    <i data-lucide="{{ $msg['icon'] }}" class="w-5 h-5 text-{{ $msg['color'] }}-600"></i>
                                </div>
                    ...
                    <div class="space-y-4">
                        @foreach ($order->orderItems as $item)
                            ...
                            <div class="flex gap-4 pb-4 border-b border-(--text-color)/10 last:border-0 last:pb-0">
                                ...
                                @else
                                    <div class="w-20 h-20 rounded-xl bg-(--card-dark) flex items-center justify-center shrink-0">
                                ...
                                <div class="flex-1">
                                    ...
                                    @endif
                                    <p class="text-(--secondary-color) font-semibold mt-2">Rs. {{ number_format($item->price, 2) }}</p>
                                    <p class="text-sm text-gray-500">Qty: {{ $item->quantity }}</p>
                                </div>
                                <p class="font-semibold text-(--text-color) shrink-0">Rs. {{ number_format($item->subtotal, 2) }}</p>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                @if ($order->shippingAddress)
                    <div class="bg-(--card-bg) rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
                        ...
                        <div class="flex gap-4">
                            ...
                            <div>
                                <p class="font-medium">{{ $order->shippingAddress->full_name ?? $order->user->name }}</p>
                                ...
                                @endif
                                <p class="text-gray-600">{{ $order->shippingAddress->city ?? '' }}{{ !empty($order->shippingAddress->state) ? ', ' . $order->shippingAddress->state : '' }}</p>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-(--card-bg) rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        <div class="flex justify-between font-semibold text-lg">
                            <span>Total</span>
                            <span class="text-(--secondary-color)">Rs. {{ number_format($order->total_amount, 2) }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        @if ($order->payment)
                            <div class="flex justify-between">
                                ...
                                <span class="{{ $paid ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }} px-3 py-0.5 rounded-full text-xs font-medium capitalize">
                            ...
                            @if ($order->payment->transaction_id ?? $order->transaction_id)
                                <div class="flex justify-between">
                                    ...
                                    <span class="font-mono text-xs">{{ $order->payment->transaction_id ?? $order->transaction_id }}</span>
                                </div>
                            @endif
                ...
                @elseif ($order->isCancellable())
                    <div class="bg-(--card-bg) rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
                        ...
                        <p class="text-gray-600 text-sm mb-5">You can cancel this order while it's still pending or confirmed.</p>
                        <form method="POST" action="{{ route('order.cancel', $order->id) }}"
                            onsubmit="return confirm('Are you sure you want to cancel this order?')">
                            @csrf
                            @method('PATCH')
                            <button type="submit"
                                class="w-full border border-red-400 text-red-500 hover:bg-red-50 py-3.5 rounded-xl font-medium transition">
                                Cancel Order
                            </button>
                        </form>
                    </div>
