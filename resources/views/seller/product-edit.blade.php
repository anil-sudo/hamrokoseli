                ...
                <div id="previewGrid" class="grid grid-cols-4 gap-4">
                    @foreach ($product->images as $image)
                        <div class="aspect-square border border-(--text-color)/20 rounded-2xl overflow-hidden relative group existing-image" data-image-id="{{ $image->id }}">
                            ...
                            <button type="button" onclick="removeExistingImage(this, {{ $image->id }})" class="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1.5 opacity-0 group-hover:opacity-100 transition-all">
            ...
            <div id="pricingSection"
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-(--text-dark) mb-2">Discount Price (Rs.)</label>
                            <input type="number" id="discounted_price" name="discounted_price" value="{{ old('discounted_price', $product->discount_price) }}" min="0"
                                class="w-full px-5 py-4 bg-(--card-dark) border border-(--bg-color)/30 rounded-xl text-base">
